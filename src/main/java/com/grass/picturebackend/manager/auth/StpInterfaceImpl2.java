package com.grass.picturebackend.manager.auth;
...
import static com.grass.picturebackend.constant.UserConstant.USER_LOGIN_STATE;

/**
 * @author Mr.Liuxq
 * @description: 自定义权限加载2
 * @date 2025年05月13日 14:11
 */
@Component
public class StpInterfaceImpl2 implements StpInterface {
    ...
    private static final ThreadLocal<SpaceUserAuthContext> authContextThreadLocal = new ThreadLocal<>();
     ...
     */
    @Override
    public List<String> getPermissionList(Object loginId, String longinType) {
        // 判断loginType，仅对space进行权限校验
        if (!StpKit.SPACE_TYPE.equals(longinType)) {
        ...
        // 如果上下文对象中没有 SpaceUser 对象，并且含有spaceUserId,则从数据库中查询
    ...
    /*
     * @description: 从请求中获取上下文对象
     * @author: Mr.Liuxq
     * @date 2025/5/13 14:28
     */
    private SpaceUserAuthContext getAuthContextByRequest() {
        // 从 ThreadLocal 中获取上下文对象
        SpaceUserAuthContext authContext = authContextThreadLocal.get();
        if (authContext != null) {
            return authContext;
        }
        ...
        // 将解析后的上下文对象设置到 ThreadLocal 中
        authContextThreadLocal.set(authRequest);
        return authRequest;
    }
