...
import com.grass.picturebackend.manager.websocket.disruptor.PictureEditEventProducer;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    /**
     * 当WebSocket连接关闭时调用此方法，处理用户退出编辑状态的相关逻辑。
     *
     * @param session 关闭的WebSocket会话对象，包含会话属性和相关信息。
     * @param status 连接关闭的状态，包含关闭原因和状态码。
     * @throws Exception 如果在处理过程中发生异常，则抛出。
     */
    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        // 获取会话中的属性，包括图片ID和用户信息
        ...
        // 从图片会话集合中移除当前会话，如果会话集合为空，则移除该图片ID的会话集合
    ...
    /**
     * 处理WebSocket接收到的文本消息。
     * 该方法根据消息类型调用相应的处理方法，处理图片编辑相关的请求。
     *
     * @param session WebSocket会话对象，包含与客户端的连接信息。
     * @param message 接收到的文本消息，包含客户端发送的请求数据。
     * @throws Exception 处理过程中可能抛出的异常。
     */
    @Override
    protected void handleTextMessage(@Nonnull WebSocketSession session, @Nonnull TextMessage message) throws Exception {
        //super.handleTextMessage(session, message);
        // 将消息解析成pictureEditRequestMessage
        ...
        /*switch (pictureEditMessageTypeEnum) {
            ...
            default:
                ...
                pictureEditResponseMessage.setUser(userService.getUserVO(user));
                session.sendMessage(new TextMessage(JSONUtil.toJsonPrettyStr(pictureEditResponseMessage)));
                break;
        }*/
    }
